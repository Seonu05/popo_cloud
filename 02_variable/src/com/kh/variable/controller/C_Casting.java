...
public class C_Casting {
	...
	//컴파일러가 자동으로 작은 자료형을 큰 자료형으로 변화 시키는 것.
	...
	public void autoCasting() {
		...
		int i2 = 2147483647;
		long l2 = i2 + 1L; 
		// 컴파일러가 2147483674L + 1L 형태로 변환해서 계산을 진행함.
		// 이것도 자동 형변환
		System.out.println("12의 값 : " + l2);
		...
		System.out.println("(출력결과)"+ f3 + " + " + d3 + " = "+ result3);
		// (출력 결과) 2.13 + 3.14 = 5.27
		
		// 10진수 유한소수를 2진수로 변환하면 무한소수가 되는 경우가 많다.
		// 그런데, 컴퓨터는 실수를 부동소수점으로 표현하기 때문에
		// 2진수 무한 소수를 정확시 표현하는데 한계가 있다.
		// 그래서 오차가 발생한다.
		
		//char(2byte) -> int (4byte)
		int i4 = 'A'; //int형 변수에 char형 리터럴 값 대입
		/* char -> int 자동 형변환이 가능한 이유
		 * char라는 자료형 변수는 문자 자체를 저장하는 변수가 아닌
		 * 2byte(0~65535) 범위 내의 숫자를 저장하고,
		 * 유니코드라는 문자표에서 해당번호와 일치하는 문자를 찾아
		 * 출력하는 자료형이다.
		...
		// byte, short, char, int
		// 이 네가지 자료형은 int 리터럴로 값을 초기화 할 수 있다.
		...
		public void forcedCasting() {
			// 값의 범위가 작은 자료형 -> 큰 자료형으로 강제로 변환 시키는 것.
			
			int iNum1 = 10;
			long lNum1 = 10;
			//프로그래밍에서 '='기호는 대입 연산자를 의미
			// -> 대입도 연산이다
			// long형 변수에 int 리커럴 값을 대입할 경우
			// int 리터럴 값이 long형의 리터럴 자동 형변환하여 저장됨.
			
			//int result = iNum1 + lNum1;
			//iNum + lNum 의 연산 결과 자료형은? long
			// int = long 대입연산 모양이 됨
